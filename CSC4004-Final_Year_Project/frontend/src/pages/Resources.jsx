    ...
    <div className="page-container">
      <div className="hubtoll-page-header">
        <div>
          <div className="hubtoll-module-eyebrow">
            <span className="hubtoll-module-badge-icon" style={{ background: '#3b82f6' }}>
            ...
            <span className="hubtoll-module-badge-text">DEPARTMENT RESOURCE & ASSET MANAGEMENT</span>
          </div>
          <h1 className="page-title">Department Resources & Assets</h1>
          <p className="page-subtitle">Categorized asset inventory, live availability status, and booking management.</p>
        </div>
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          ...
          {!isStaff && (
            <button className="hubtoll-btn-primary" type="button" onClick={openAddResource}>
              + Add Resource
            </button>
          )}
        </div>
      ...
      <div className="hubtoll-kpi-grid">
        <div className="hubtoll-kpi-card accent-blue">
          <div className="hubtoll-kpi-label">TOTAL ASSETS</div>
          <div className="hubtoll-kpi-value">{resources.length}</div>
          <div className="hubtoll-kpi-sub">Registered department equipment & rooms</div>
        </div>

        <div className="hubtoll-kpi-card accent-green">
          <div className="hubtoll-kpi-label">AVAILABLE FOR USE</div>
          <div className="hubtoll-kpi-value" style={{ color: 'var(--color-success)' }}>{availableCount}</div>
          <div className="hubtoll-kpi-sub">Ready for immediate allocation</div>
        </div>

        <div className="hubtoll-kpi-card accent-orange">
          <div className="hubtoll-kpi-label">IN ACTIVE USE</div>
          <div className="hubtoll-kpi-value" style={{ color: '#ea580c' }}>{inUseCount}</div>
          <div className="hubtoll-kpi-sub">Currently deployed or booked</div>
        </div>

        <div className="hubtoll-kpi-card accent-red">
          <div className="hubtoll-kpi-label">INSPECTION / REPAIR</div>
          <div className="hubtoll-kpi-value" style={{ color: 'var(--color-danger)' }}>{maintenanceCount}</div>
          <div className="hubtoll-kpi-sub">{maintenanceCount === 0 ? 'All assets healthy' : 'Currently in maintenance'}</div>
        </div>
      </div>
